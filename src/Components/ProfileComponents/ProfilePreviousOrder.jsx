  ...
  useEffect(() => {
    const getorders = async () => {
      ...
      const orders = await fetch(
        `http://localhost:3000/api/orders/getcustomerorders?customerid=${_id}`,
      ...
      <div className="grid grid-cols-5 md:grid-cols-1 sm:grid-cols-1 place-center gap-5 max-h-content">
        {orders &&
          orders.map((order, index) => (
            ...
            >
              <div className="font-bold text-2xl text-green-900 text-center">Order Number <br/> {index}</div>
